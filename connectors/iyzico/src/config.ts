 ...
 * future retry/timeout layer (ADR-016) can wrap it without changing callers.
 */
...
export type IyzicoConfig = {
   ...
   * double, a smoke-test passes the real `fetch`, and ADR-016 retry/timeout will
   * wrap this same seam — so testability and infrastructure enter through one door.
   */
  fetch?: FetchLike;
};
